...
type Finder interface {
	...
	RangeRevert(scope Range) ([]interface{}, error)
}
...
func (rr *ReferenceResult) RangeRevert(scope Range) *ReferenceResult {
	rr.times = rr.times + 1
	if strs, err := rr.db.RangeRevert(scope); err == nil {
		rr.set.SortAdd(rr.times, strs...)
	} else {
		rr.err = err
	}
